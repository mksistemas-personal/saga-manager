...
@Transactional
@AllArgsConstructor
class UnlinkStateStepWithStateStepService implements UnlinkStateStepWithStateStepUseCase {
    ...
    private Either<? extends BusinessException, Tsid> save(Context context) {
        Tsid stateId = stateStepRepository.unlinkStateStepWithStateStep(context.sourceId(), context.destId());
    ...
    private Either<BusinessException, Context> checkStates(final Context context) {
        ...
        if (!stateStepRepository.existsStateStepRelationship(context.sourceId(), context.destId()))
            return Either.left(new BusinessException(SagaConstants.STEP_RELATIONSHIP_NOT_FOUND));
